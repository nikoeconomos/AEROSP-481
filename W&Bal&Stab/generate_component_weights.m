...
%                                  v1: 10/20/24
    ...
    % locations/MAC
    wing.xRLE = 7.083; %m positino of leading edge of the root chord, from CAD
    ...
    f.fore_pct       = f.fore       / f.total_used; % percent
    f.center_pct     = f.center     / f.total_used;
    f.aft_pct        = (f.aft-remainder) / f.total_used; %because we have a bit over what we need
