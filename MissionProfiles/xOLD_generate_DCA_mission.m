...
DCA_mission.start_takeoff.time = .1; % averaged this from data online (~6 min)

% Climb segment
DCA_mission.climb.ff = 0.985; % [unitless]
DCA_mission.climb.time = 1/60; % [hours] - overestimate from data online (~1 min)
...
DCA_mission.cruise_out.time = (DCA_mission.cruise_out.range / (DCA_mission.cruise_out.mach*343)) / 3600; % time [s] = distance [m] / speed [m/s] - then convert to hours
...
DCA_mission.loiter.time = 4; % [hours] - given in RFP mission description
...
DCA_mission.dash.time = (DCA_mission.dash.range / (DCA_mission.dash.mach*343)) / 3600; % time [s] = distance [m] / speed [m/s] - then convert to hours
...
DCA_mission.combat1.time = (DCA_mission.combat1.range / (DCA_mission.combat1.mach*343)) / 3600; % time [s] = distance [m] / speed [m/s] - then convert to hours
...
DCA_mission.combat2.time = (DCA_mission.combat2.range / (DCA_mission.combat2.mach*343)) / 3600; % time [s] = distance [m] / speed [m/s] - then convert to hours
DCA_mission.combat2.time = DCA_mission.combat2.time + .01;% adding buffer time to release all missles (approx 36 seconds)
...
DCA_mission.cruise_in.time = (DCA_mission.cruise_in.range / (DCA_mission.cruise_in.mach*343)) / 3600; % time [s] = distance [m] / speed [m/s] - then convert to hours


% Descent segment
DCA_mission.descent.ff = 0.990; % [unitless]
DCA_mission.decent.time = 4 / 60; % averaged historical data for decent time [hours]
